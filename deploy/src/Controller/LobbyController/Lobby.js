...
startButton.addEventListener("click", function() { DAL.updateGameState(Config
    .GAME_STARTED); });
...
function init() {
  ...
  roundCountSetting.value = roundCountRangeSetting.value = Config
  .DEFAULT_ROUNDS;
  roundDurationSetting.value = roundDurationRangeSetting.value = Config
    .DEFAULT_ROUND_DURATION;

  //Round Count
  roundCountRangeSetting.addEventListener("input", function() { DAL
      .updateSessionWithSettings(roundCountRangeSetting.value, null); });
  roundCountSetting.addEventListener("input", function() { DAL
      .updateSessionWithSettings(roundCountSetting.value, null); });
  //Round Duration
  roundDurationRangeSetting.addEventListener("input", function() { DAL
      .updateSessionWithSettings(null, roundDurationRangeSetting.value); });
  roundDurationSetting.addEventListener("input", function() { DAL
      .updateSessionWithSettings(null, roundDurationSetting.value); });
